...
instance Ord Country where
  ...
  compare _ Switzerland = LT  -- implement me?
  Finland <= _ = True
  Norway <= Norway = True
  _ <= Switzerland = True -- and me?
  min Finland _ = Finland -- and me?
  min Switzerland Norway = Norway
  max Norway Finland = Norway
  max Switzerland _ = Switzerland  -- and me?
...
instance Eq Name where
  Name x == Name b = map toLower x == map toLower b
                     || x == b
                     || map toLower x == b
                     || x == map toLower b  
...
instance Eq a => Eq (List a) where
  Empty == Empty = True
  Empty == LNode x (b) = False
  LNode x b == Empty = False
  LNode x (b) == LNode y (c) = x == y && b == c -- compares both the node and the next element
...
instance Ord Number where
  ...
  Infinite > (Finite x) = True
  (Finite x) > Infinite = False
  (Finite x) > (Finite y)  = x > y
  
  min (Finite x) Infinite  = (Finite x)
  min Infinite (Finite x)  = (Finite x)
  min (Finite x) (Finite y) = if x >= y then Finite y else Finite x
  max (Finite x) (Finite y) = if x <= y then Finite y else Finite x
  max _ Infinite = Infinite
  max Infinite _ = Infinite

-- model answer of "minimal complete definition"
-- instance Ord Number where
--  (Finite x) <= (Finite y) = x <= y
--  _          <= Infinite   = True
--  _          <= _          = False
...
instance Num RationalNumber where
  RationalNumber a b + RationalNumber c d = 
    simplify (RationalNumber ((a*d) + (c*b)) (b*d))
      where simplify (RationalNumber a b)
              | (gcd a b) <= b = (RationalNumber (a `div` (gcd a b)) (b `div` (gcd a b)))
  ...
  abs (RationalNumber a b)
    | a < 0 = RationalNumber (a*(-1)) b
    | b < 0 = RationalNumber a (b*(-1))
    | (a < 0 && b < 0)  = RationalNumber (a*(-1)) (b*(-1))
    | otherwise = RationalNumber a b
  
  signum (RationalNumber a b) = (RationalNumber (check a) (check b))
                                  where check x
                                         | x == 0 = 0
                                         | x > 0  = 1
                                         | x < 0  = (-1)
...
class Cycle a where
  step :: a -> a
  stepMany :: (Num b, Eq b, Ord b) => b -> a -> a
  stepMany 0 suit = suit
  stepMany x suit
    | x < 0 = suit
    | otherwise = stepMany (x-1) (step suit) 

...
prettyPrint n queens = fullGrid n [queen c | c <- [(x,y) | x <- [1..n], y <- [1..n]]]
                where queen (a,b)
                        | elem (a,b) queens = 'Q'
                        | otherwise = '.'
                      fullGrid n xs = insert n '\n' xs
                      insert n char [] = []
                      insert n char xs
                        | length xs < n = xs
                        | otherwise = take n xs ++ [char] ++ insert n char (drop n xs)
...
danger cand []     = False
danger cand (x:xs)
    | elem True [sameRow cand x, sameCol cand x,
        sameDiag cand x, sameAntidiag cand x] = True
...
prettyPrint2 n queens = fullGrid n [queen c | c <- [(x,y) | x <- [1..n], y <- [1..n]]]
                where queen (a,b)
                        | elem (a,b) queens = 'Q'
                        | danger (a,b) queens = '#'
                        | otherwise = '.'
...
fixFirst 0 _ = Nothing
fixFirst n [(r,c)]
 | c > n = Nothing
 | otherwise = Just [(r,c)]
fixFirst n ((r,c):y:queens)
  | c > n = Nothing
  | otherwise =
    if all (== False) (map (`danger` [(r,c)]) (y:queens))
    then Just ((r,c):y:queens)
    else fixFirst n ((r,c+1):y:queens)
...
continue (a:queens) = check (nextRow a:a:queens)
                        where check (a:b:cs)
                                | (fst a) == (fst b) = check (nextRow a:b:cs)
                                | otherwise = (a:b:cs)

backtrack :: Stack -> Stack
backtrack (a:b:queens) = (nextCol b:queens)
...
finish 0 _      = []
finish n queens
  | length queens /= (n+1) = finish n (step n queens)
  | otherwise = tail queens

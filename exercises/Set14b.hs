...
openDatabase path = do
  connect <- open path
  execute_ connect initQuery
  return connect

openDatabase' path = open path >>= 
                     \connect -> execute_ connect initQuery >> 
                     return connect 
...
balance db name = do
  allTx <- query_ db getAllQuery :: IO [(String,Int)]
  let allDeps = filter (\x -> fst x == T.unpack name) allTx
  return (sum $ map snd allDeps)
...
parseCommand [command,name] = case fmap T.unpack [command,name] of
                                      ["balance",name2] -> Just (Balance name)
                                      _ -> Nothing
parseCommand [command,name,value] = case fmap T.unpack [command,name] of
                                      ["deposit",name2] -> parseInt value >> 
                                                            Just (Deposit name (read $ T.unpack value :: Int))
                                      ["withdraw",name2] -> parseInt value >> 
                                                              Just (Withdraw name (read $ T.unpack value :: Int))
                                      _         -> Nothing
...
perform db (Just (Deposit name val)) = do
  deposit db name val
  return (T.pack "OK")
...
perform db (Just (Withdraw name val)) = do
  a <- T.unpack <$> perform db (Just (Balance name))
  perform db (Just (Deposit name (negate val)))
  balance db name
  return (T.pack "OK")
...
server db request respond = do
  let path = pathInfo request
      a =  parseCommand path
  b <- perform db a
  respond (responseLBS status200 [] (encodeResponse b))

...
workload nExercises hoursPerExercise
  | (hoursPerExercise * nExercises) > 100 = "Holy moly!"
  | (hoursPerExercise * nExercises) < 10  = "Piece of cake!"
  | otherwise = "Ok."
...
sumSuccess = foldr addRight (Left "no data")
              where addRight (Left _) (Left x)  = Left x
                    addRight (Left _) (Right x) =  Right x
                    addRight (Right x) (Right y) = Right (x+y)
                    addRight (Right x) (Left _) = Right x


            
sumSuccess' :: [Either String Int] -> Either String Int
sumSuccess' xs = go xs leftOrRight
    where leftOrRight = Left "no data"
          go [] leftOrRight = leftOrRight
          go ((Left x):xs) leftOrRight = go xs leftOrRight
          go ((Right x):xs) (Left _) = go xs (Right x)
          go ((Right x):xs) (Right y) = go xs ((+) <$> Right x <*> Right y)
...
changeCode x (Open y)   = Open x
changeCode x (Locked y) = Locked y
...
instance Eq Text where
  (Text x) == (Text y) = ignoreSpace x == ignoreSpace y

ignoreSpace :: [Char] -> [Char]
ignoreSpace = filter (not . isSpace)
...
compose [] _ = []
compose ((r,k1):xs) list =
  case lookup k1 list of
    (Just v) -> (r,v): compose xs list
    Nothing  -> compose xs list
...
permute [] _ = []
permute (pos:xs) (char:ys) =
  let  letterPos :: Permutation -> [a] -> [(Int, a)]
       letterPos [] _ = []
       letterPos (pos:xs) (char:ys) = (pos, char) : letterPos xs ys
       sortPos :: [(Int,a)] -> [(Int, a)]
       sortPos [] = []
       sortPos ((x,v1):pairs) =
         let smallPos = sortPos [(a,v) | (a,v) <- pairs, a <= x]
             bigPos   = sortPos [(a,v) | (a,v) <- pairs, a > x]
          in smallPos ++ [(x,v1)] ++ bigPos
       removePos :: [(Int, a)] -> [a]
       removePos [] = []
       removePos ((a,b):pairs) = b : removePos pairs
   in  removePos $ sortPos $ letterPos (pos:xs) (char:ys)

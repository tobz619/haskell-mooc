...
totalPrice (MkShoppingEntry a b c) = b * fromIntegral c 
...
setName name (Person a b) = (Person a name)

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age (Person a b) = (Person age b) 
...
rgb (Invert col)   =  map (abs . negate . (\x -> x-1)) (rgb col)
...
toNat z
  | z < 0     = Nothing
  | z == 0    = Just Zero
  | otherwise = Just (pluses z)
                  where pluses 1 = (PlusOne Zero)
                        pluses n = (PlusOne (pluses (n-1)))
...
--      |- This parameter accumulates increments of one
--      |      |- The binary number
--      |      |       |- The result
go   :: Int -> Bin ->  Int
go _ End   = 0
go a (I n) = go (a+1) n + 2 ^ a
go a (O n) = go (a+1) n

...
allEqual xs = 
    let check = map (== (head xs)) xs
    in  if all (== True) check then True else False
------------------------------------------------------------------------------
...
rangeOf xs =  (minimum xs) `subtract` (maximum xs)
...
winner scores player1 player2 = 
    let winScore = getScore scores player1 player2
    in  if Map.findWithDefault 0 player1 scores == winScore
        then player1
        else if Map.findWithDefault 0 player2 scores == winScore
             then player2
             else player1

getScore :: Map.Map String Int -> String -> String -> Int
getScore m a b =
    let scoreA = case Map.lookup a m of Just x -> x
                                        Nothing -> 0
        scoreB = case Map.lookup b m of Just x -> x
                                        Nothing -> 0
    in  if scoreA > scoreB
        then scoreA
        else scoreB
...
freqs []      = Map.empty
freqs (x:xs)  = foldr someFunc Map.empty (x:xs)
                where someFunc x m = Map.alter addOne x m
                      addOne Nothing  = Just 1
                      addOne (Just v) = Just (v+1)


freqs' :: (Eq a, Ord a) => [a] -> Map.Map a Int
freqs' xs = 
    let indiCount a = (a, (length $ filter ( == a) xs))
        values      = nub xs
    in  Map.fromList $ map indiCount values
...
transfer from to amount bank
 ...
 | Map.lookup from bank < (Just 0)    = bank
 | Map.lookup to bank == Nothing      = bank
 | Map.lookup from bank == Nothing    = bank
 | Just amount < Just 0               = bank
 | otherwise =  let deduct  = Map.alter (\(Just f) -> Just (f-amount)) from bank
                in  Map.alter (\(Just t) -> (Just (t+amount))) to deduct
...
maxIndex arr = checkMax (indices arr)
            where checkMax (a:as)  = if (arr ! a) == bigEl
                                     then a
                                     else checkMax as
                  bigEl            = maximum $ elems arr

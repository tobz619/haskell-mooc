...
count x = foldr (\v -> if x == v then (+1) else (+0)) 0
...
inBoth a b = foldr (:) [] a `intersect` foldr (:) [] b

inBoth' :: (Foldable f, Foldable g, Eq a) => f a -> g a -> [a]
inBoth' a b = foldr keep [] a
  where keep x xs
         | elem x b = x:xs
         | otherwise = xs
------------------------------------------------------------------------------
...
instance Foldable List where
  foldr f z Empty = z
  foldr f z (LNode a b) = f a (foldr f z b)
...
instance Foldable TwoList where
  ...
  foldr f z (TwoNode a b c) = f a (f b (foldr f z c))

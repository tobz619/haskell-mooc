...
greetText xs
    | T.length xs > 15 = T.pack $ "Hello, " ++ take 15 (T.unpack xs) ++ "...!"
    | otherwise = T.pack $ "Hello, " ++ T.unpack xs ++ "!"
...
shout :: T.Text -> T.Text
shout =  T.pack . unwords . everySnd . words . T.unpack
    where everySnd [] = []
          everySnd [x] = [map toUpper x]
          everySnd (x:y:xs) = map toUpper x : y : everySnd xs
...
longestRepeat = seeString . T.unpack
        where seeString "" = 0
              seeString xs =
                  let makeReps [] acc = [acc]
                      makeReps [x] acc = [acc+1]
                      makeReps (x:y:xs) acc
                        | x == y = makeReps (y:xs) (acc+1)
                        | otherwise = acc+1 : makeReps (y:xs) 0
                   in maximum $ makeReps xs 0
...
xorChecksum = pair 0 . B.unpack
        where   pair acc [] = acc
                pair acc (x:xs) = xor acc (pair x xs)
...
countUtf8Chars chs = case decodeUtf8' chs of
                        (Left _) ->  Nothing
...
pingpong' xs =
    let unpacked = B.unpack
        cycled   = cycle (unpacked xs ++ reverse (unpacked xs))
     in BL.pack $ cycled

pingpong :: B.ByteString -> BL.ByteString
pingpong = BL.pack . (\xs -> cycle (xs ++ reverse xs)) . B.unpack
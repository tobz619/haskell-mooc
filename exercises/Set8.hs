...
combine f (Picture p1) (Picture p2) =
  let getCoordCol coord =
       let px1Col = p1 coord
           px2Col = p2 coord
        in f px1Col px2Col
  in  Picture (getCoordCol)

combinek :: (Color -> Color -> Color) -> Picture -> Picture -> Picture
combinek f (Picture coordToColor1) (Picture coordToColor2) =
  let fromCoordToBlendedColor :: Coord -> Color
      fromCoordToBlendedColor coord =
       let colorOfPixel1 = coordToColor1 coord
           colorOfPixel2 = coordToColor2 coord
       in  f colorOfPixel1 colorOfPixel2
  in Picture fromCoordToBlendedColor


combinel :: (Color -> Color -> Color) -> Picture -> Picture -> Picture
combinel f (Picture x) (Picture y) =
  Picture (\coord -> x coord `f` y coord)
...
rectangle x0 y0 w h = Shape f
   where f (Coord x y) = let recCoords :: [(Int, Int)]
                             recCoords = let tLeft  = (x0, y0)
                                             bRight = (x0 + w, y0 + h)
                                          in [tLeft, bRight]
                             xVals = map fst recCoords
                             yVals = map snd recCoords
                             eligXs = (x >= minimum xVals) && (x < maximum xVals)
                             eligYs = (y >= minimum yVals) && (y < maximum yVals)
                          in eligXs && eligYs
...
union (Shape s1) (Shape s2) = Shape f
  where f (Coord x y) = let shapeCoords :: Coord -> Bool
                            shapeCoords c =
                              let sCo1 = s1 c
                                  sCo2 = s2 c
                              in  (sCo1 || sCo2) == True
                        in shapeCoords (Coord x y)
...
cut (Shape s1) (Shape s2) = Shape f
  where f (Coord x y) = let shapeCoords :: Coord -> Bool
                            shapeCoords c =
                              let sCo1 = s1 c
                                  sCo2 = s2 c
                              in  if sCo1 == sCo2 then False else True
                        in  shapeCoords (Coord x y)
...
paintSolid color (Shape s1) (Picture ori) = Picture f
  where f (Coord x y) = let getCoord :: Coord -> Bool
                            getCoord c
                             | s1 c  = True
                             | otherwise = False
                            eligCoord = getCoord (Coord x y)
                            picCol :: Coord -> Color
                            picCol c =
                              let pixCol = ori c
                              in  pixCol
...
paint (Picture new) (Shape s1) (Picture ori) = Picture f
  where f (Coord x y) = let getCoord :: Coord -> Bool
                            getCoord c
                              | s1 c = True
                              | otherwise = False
                            eligCoord = getCoord (Coord x y)
                            useCol :: (Coord -> Color) -> Coord -> Color
                            useCol x c =
                                let pixCol = x c
                                in  pixCol
                          in if eligCoord
                             then useCol new (Coord x y)
                             else useCol ori (Coord x y)
...
instance Transform Fill where
  apply (Fill c) (Picture p) = Picture (\coord -> c)
...
instance Transform Zoom where
  apply (Zoom z) (Picture p) = Picture f
    where f = let getCoord :: Coord -> Color
                  getCoord c =
                    let coordCol = p c
                    in  coordCol
                  zoomCo :: Int -> Coord -> Coord
                  zoomCo z (Coord x y) = Coord (x `div` z) (y `div` z)
                  zoomPic :: Int -> Picture -> (Coord -> Color)
                  zoomPic z (Picture p) = (getCoord . zoomCo z)
               in zoomPic z (Picture p)
...
instance Transform Flip where
  apply FlipX (Picture p) = Picture f
    where f = let getCol :: Coord -> Color
                  getCol c =
                    let coCol = p c
                    in  coCol
                  fXCoord :: Coord -> Coord
                  fXCoord (Coord x y) = Coord (negate x) y
              in  (getCol . fXCoord)
  ...
  apply FlipY (Picture p) = Picture f
    where f = let getCol :: Coord -> Color
                  getCol c =
                    ...
                    in  coCol
                  fYCoord :: Coord -> Coord
                  fYCoord (Coord x y) = (Coord x (negate y))
              in  (getCol . fYCoord)
  ...
  apply FlipXY (Picture p) = Picture f
    where f = let getCol :: Coord -> Color
                  getCol c =
                    let coCol = p c
                    in  coCol
                  fXYCoord :: Coord -> Coord
                  fXYCoord (Coord x y) = (Coord y x)
              in  (getCol . fXYCoord)
...
instance Transform Blur where
  apply Blur (Picture p1) = Picture f
    where f (Coord x y) = let getCol :: Coord -> Color
                              getCol c =
                                let pixCol = p1 c
                                in  pixCol
                              getAdjCols :: Coord -> [Color]
                              getAdjCols (Coord x y) =
                                let listCols =
                                      map getCol [Coord x y,
                                      Coord (x+1) y,
                                      Coord (x-1) y,
                                      Coord x (y+1),
                                      Coord x (y-1)]
                                in listCols
                              allCols = getAdjCols (Coord x y)
                              avgCols :: [Color] -> Color
                              avgCols cols =  go cols totalCol listLength
                                where listLength = 0
                                      totalCol = Color 0 0 0
                                      go :: [Color] -> Color -> Int -> Color
                                      go [] (Color r g b) listLength =
                                        Color (div r listLength)
                                               (div g listLength)
                                               (div b listLength)
                                      go (Color r g b: cols) totalCol listLength  =
                                          go cols (colourSum totalCol (Color r g b)) (listLength + 1)
                                       where colourSum (Color r1 g1 b1) (Color r2 g2 b2) =
...
--                       in Color (sum (map getRed colors) `div` n) 
--                                (sum (map getGreen colors) `div` n) 
--                                (sum (map getBlue colors) `div` n)       
------------------------------------------------------------------------------
...
instance Transform BlurMany where
  apply (BlurMany 0) (Picture p) = Picture p
  apply (BlurMany n) (Picture p) =
    apply (Chain (BlurMany (n-1)) Blur) (Picture p)
------------------------------------------------------------------------------

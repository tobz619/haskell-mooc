...
calculator "negate" val = fmap negate (readMaybe val)
calculator "double" val = readMaybe val >>= \x -> Just (x*2)
...
validateAddress streetName streetNumber postCode =
  check (length streetName <= 20) "Invalid street name" (Address streetName streetNumber postCode) *>
   check ((not . any isAlpha) streetNumber) "Invalid street number" (Address streetName streetNumber postCode) *>
    check ((length postCode == 5) && all isDigit postCode) "Invalid postcode" (Address streetName streetNumber postCode)
...
boolOrInt x = check ((readMaybe x :: Maybe Bool) /= Nothing ) "Not a Bool" (Left (read x :: Bool)) <|>
               check ((readMaybe x :: Maybe Int) /= Nothing ) "Not an Int" (Right (read x :: Int))
...
normalizePhone xs = check (length (unspace xs) < 11) "Too long" xs *> traverse checkNum (unspace xs)
                    where unspace = filter (not . isSpace)
                          checkNum x = check (isNumber x) ("Invalid character: " ++  [x]) x
------------------------------------------------------------------------------
...
checkLength s = check (length (words s) == 3) ("Invalid expression: " ++ s) s

parseOp :: [String] -> Validation Expression
parseOp [x,"+",y] = Plus <$> parseArg x <*> parseArg y
parseOp [x,"-",y] = Minus <$> parseArg x <*> parseArg y
parseOp [x,o,y] = parseArg x *> invalid ("Unknown operator: " ++ o) <* parseArg y
parseOp _ = empty

parseExpression :: String -> Validation Expression
parseExpression s = parseOp (words s) <* checkLength s



parseArg :: String -> Validation Arg
parseArg s = check (all isNumber s) ("Invalid number: " ++ s) (Number (read s :: Int))
               <|> varChecker s
               where varChecker [a] = check (isAlpha a) ("Invalid variable: " ++ [a]) (Variable a)
                     varChecker inv = invalid ("Invalid variable: " ++ inv)
...
instance Applicative Priced where
  pure = Priced 0
  liftA2 f (Priced i1 a1) (Priced i2 a2) = Priced (i1 + i2) (a1 `f` a2)

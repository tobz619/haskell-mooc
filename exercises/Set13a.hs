...
split str
  | not (any isSpace str) = Nothing
  | otherwise = separate str
      where separate xs =
              let firstWord (x:xs) = takeWhile (/= ' ') (x:xs)
                  secondWord (x:xs) = tail $ dropWhile (/= ' ') (x:xs)
               in Just (firstWord xs, secondWord xs)
...
checkNumber (fName,sName)
 | any isNumber fName || any isNumber sName = Nothing
 | otherwise = Just (fName,sName)
...
checkCapitals (for,sur)
  | (not . isUpper) (head for) || (not . isUpper) (head sur) = Nothing
...
winner scores player1 player2 =
  let getScores p = lookup p scores
      compareScores (a, b)
        | a == Nothing || b == Nothing = Nothing
        | a >= b = Just player1
        | otherwise  = Just player2
   in compareScores (getScores player1, getScores player2)
...
selectSum xs [] = Just 0
selectSum xs (i:is) = saferIndex xs i >>= (\x -> Just (+x) <*> selectSum xs is)

saferIndex :: [a] -> Int -> Maybe a
saferIndex [] _ = Nothing
saferIndex (a:as) i
  | i < 0          = Nothing
  | i > length as  = Nothing
  | otherwise      = Just ((a:as) !! i)
...
countAndLog f (x:xs) = loggerAcc (Logger [] 0) (x:xs)
                        where loggerAcc (Logger _ a) [] = return a
                              loggerAcc (Logger str a) (x:xs)
                                | f x = Logger [show x] (a+1) >>=
                                          \b -> loggerAcc (Logger str b) xs
                                | otherwise = loggerAcc (Logger str a) xs
...
paren '(' = get >>= \x -> if x < 0 then put x else put (x+1)
paren ')' = get >>= \x -> if x < 0 then put x else put (x-1)
...
count x = get >>=
          \list -> modify (incrementIfPresent x)

incrementIfPresent :: Eq a => a -> [(a, Int)] -> [(a, Int)]
incrementIfPresent x [] = [(x,1)]
incrementIfPresent x ((a,y):tuples)
 | x == a = (a,y+1):tuples
 | otherwise = (a,y):incrementIfPresent x tuples
...
occurrences' (x:xs) = get >>=
                     \list -> put (foldr incrementIfPresent list (x:xs)) >>
                     get >>=
                     \nuList -> return ((length . nub) nuList)

occurrences :: (Eq a) => [a] -> State [(a,Int)] Int
occurrences xs = mapM_ count xs >>
                 get >>=
                 \counts -> return (length counts)

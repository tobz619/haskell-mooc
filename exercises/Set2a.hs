...
updateAt :: Int -> a -> [a] -> [a]
updateAt 0 i (_:xs) = (i:xs)
updateAt q i (x:xs) = take q (x:xs) ++ [i] ++ drop q (tail (x:xs))
...
greet first last = "Hello, " ++  first ++ (if last == Nothing then "!" else " " ++ lastName ++ "!")
    where
        lastName = maybe "" id last
...
addEithers (Right k) (Right l) = Right (k+l)

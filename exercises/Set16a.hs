...
outputInInput input output =
  forAll (elements output) (\(a,i) -> length (filter (==a) input) == i)
...
frequenciesProp freq (NonEmpty input) =  conjoin [sumIsLength input x,
                                                 inputInOutput input x,
                                                 outputInInput input x]
                                          where x = freq input

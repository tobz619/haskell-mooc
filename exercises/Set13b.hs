...
ifM opBool opThen opElse = opBool >>=
                           \bool -> if bool
                                    then opThen
                                    else opElse
...
visit maze place = do
              curState <- get
              if elem place curState
              then put curState
              else do modify (place :)
                      newState <- get
                      let nbrs = getAdjRooms maze place
                      mapM_ (visit maze) nbrs
...
instance Functor SL where
  -- implement fmap
  fmap f (SL g) = SL $ \state ->
    let (newState , int , l) = g state
     in (f newState, int, l)
...
instance Monad SL where
  -- implement return and >>=
  return x = SL $ \int -> (x , int , [])
  SL h >>= f = SL $ \s -> let (state, int, log) = h s
                              SL g = f state
                              (newState, newInt, newLog) = g int
                           in (newState, newInt, log ++ newLog)

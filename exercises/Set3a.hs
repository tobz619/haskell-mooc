...
whileRight step x = case step x of
    Left a -> a
    Right b -> whileRight step b
...
mapper :: Either a Int -> Int
mapper (Left _)  = 0  -- this is equal to either p1
mapper (Right b) = b  -- this is equal to either p2 -> together they make 
                                    -- either p1 p2 :: Either a b -> c
...
multiCompose (f:fs) x    = foldr1 (.) (f:fs) x
...
interpreter []          = []
interpreter commands    = aux (0,0) commands


aux :: (Int, Int) -> [String] -> [String]
aux p []             = []
aux p ("up":xs)      = aux (p `addPoint` (0,1)) xs
